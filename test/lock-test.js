...
describe("Lock Contract",function(){ 
    ...
    it("revert ecxeeding transfer" ,async function(){
        await expect(token.connect(user1).transfer(user2.address, ethers.utils.parseUnits("300",18)));
    });
    describe("Contract Functions", function () {
        ...
        it("user2 cannot withdraws tokens", async function(){
        ...
        it("user1 cannot withdraws more tokens", async function(){
            await expect(lock.connect(user1).withdrawTokens()).to.be.reverted;
        })
    })
    // describe("Contract Funcitons", function(){
